...
//configure logger
const context = {context: "selection.js"};
var locker_selected = 0;

//send to main
function toIndex(){
    app_logger.info(context, `Regresando a pantalla index`);
    ipcRenderer.send('page:index', null);
}

//select locker
function lockerSelect(locker_number){
    ipcRenderer.send('locker:select', locker_number);
    app_logger.info(context, `Locker ${msj.name} seleccionado`);
}
...
ipcRenderer.on('locker:user', (msj) => {
    ...
    app_logger.info(context, `Usuario: ${msj.name} ${msj.lastname}`);
    success_alert_txt.innerText = `Bienvenido ${msj.name} ${msj.lastname}. \r\nElige un casilero desocupado.`;
    success_alert.classList.remove("hidden");
    success_alert.classList.add("rotate-horizontal-center");
    setTimeout(function () {
        success_alert.classList.add("hidden");
...
locker1.addEventListener('click', function(){lockerSelect(1)});
locker2.addEventListener('click', function(){lockerSelect(2)});
locker3.addEventListener('click', function(){lockerSelect(3)});
locker4.addEventListener('click', function(){lockerSelect(4)});
locker5.addEventListener('click', function(){lockerSelect(5)});
locker6.addEventListener('click', function(){lockerSelect(6)});
locker7.addEventListener('click', function(){lockerSelect(7)});
locker8.addEventListener('click', function(){lockerSelect(8)});
locker9.addEventListener('click', function(){lockerSelect(9)});
locker10.addEventListener('click', function(){lockerSelect(10)});
locker11.addEventListener('click', function(){lockerSelect(11)});
locker12.addEventListener('click', function(){lockerSelect(12)});
locker13.addEventListener('click', function(){lockerSelect(13)});
locker14.addEventListener('click', function(){lockerSelect(14)});
locker15.addEventListener('click', function(){lockerSelect(15)});
locker16.addEventListener('click', function(){lockerSelect(16)});